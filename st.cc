...
void st()
{
  ...
  //eventVars.Add("/export/cmss/acalamba/Wg/*.root/eventVars");
  ...
  //selectedObjects.Add("/export/cmss/acalamba/Wg/*.root/selectedObjects");
  ...
  //allObjects.Add("/export/cmss/acalamba/Wg/*.root/allObjects");
  ...
  while(allObjects.GetEntry(iEntry++) != 0){
    ...
    for (unsigned i(0);i<e.size;i++) {
       MT = (mini::mt(met,metPhi,e.pt[i],e.phi[i])>MT) ? mt(met,metPhi,e.pt[i],e.phi[i]) : MT;
    }
    for (unsigned i(0);i<m.size;i++) {
       MT = (mini::mt(met,metPhi,m.pt[i],m.phi[i])>MT) ? mt(met,metPhi,m.pt[i],m.phi[i]) : MT;
    }
...
TFile fout("st_plots_photonhad_mt.root", "recreate");
hlist.Write();
fout.Close();
