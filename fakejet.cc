...
void fakejet()
{
  ...
  while(allObjects.GetEntry(iEntry++) != 0){
    ...
    std::set<unsigned int> jf1 = JetFake1(rp);
    std::set<unsigned int> jf2 = JetFake2(rp);
    std::set<unsigned int> jf3 = JetFake3(rp);
    ...
    if(jf1.size()>0) {
      for (std::set<unsigned int>::iterator it=jf1.begin();it!=jf1.end();++it) {
        h_jf1_pt->Fill(rp.pt[*it]);
      }
    }
    if(jf2.size()>0) {
      for (std::set<unsigned int>::iterator it=jf2.begin();it!=jf2.end();++it) {
        h_jf2_pt->Fill(rp.pt[*it]);
      }
    }
    if(jf3.size()>0) {
      for (std::set<unsigned int>::iterator it=jf3.begin();it!=jf3.end();++it) {
        h_jf3_pt->Fill(rp.pt[*it]);
      }
    }
